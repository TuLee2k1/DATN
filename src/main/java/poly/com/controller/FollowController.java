...
//    @GetMapping("/toggle-follow-job/{jobPostId}")
//    public ResponseEntity<Map<String, String>> toggleFollowJob2(HttpSession session, @PathVariable Long jobPostId, Model model) {
//        try {
//            // Kiểm tra người dùng đã đăng nhập hay chưa
//            User currentUser = authenticationUtil.getCurrentUser();
//            if (currentUser == null) {
//                Map<String, String> response = new HashMap<>();
//                model.addAttribute("message", "Vui lòng đăng nhập trước khi thực hiện thao tác này.");
...
//
//            Map<String, String> response = new HashMap<>();
//            model.addAttribute("message", "Đã thêm vào danh sách theo dõi");
...
//    }
    ...
    public ResponseEntity<Map<String, String>> toggleFollowJob(HttpSession session, @PathVariable Long jobPostId) {
        try {
            ...
            if (currentUser == null) {
                Map<String, String> response = new HashMap<>();
                response.put("error", "Vui lòng đăng nhập trước khi thực hiện thao tác này.");

                return ResponseEntity.status(401).body(response);
            }

            // Lưu trạng thái theo dõi
            Follow follow = followService.toggleFollowJobPost(jobPostId);

            Map<String, String> response = new HashMap<>();
            if (follow == null) {
                // Nếu không có Follow (nghĩa là đã unfollow)
                response.put("message", "Unfollow");
            } else {
                // Nếu có Follow (nghĩa là đã follow)
